...
export default function FarmerVisit(props) {
...
console.log(resultArray);
  ...
  const onFilterChange1 = async(filterModel) => {
    ...
    try{  
      ...
      if(filterModel){
      const response = await window.Platform.database.getFarmerVisitFilter({filterField:filterModel.field,filterValue:filterModel.value,pageNumber:paginationModel.page,startDate:startDate,endDate:endDate,territoryName:territoryFilter })
      ...
      setRowCount(response.count[0].count)
        ...
        const response = await window.Platform.database.getFarmerVisitFilter({filterField:"territory",filterValue:"",pageNumber:paginationModel.page,startDate:startDate,endDate:endDate,territoryName:territoryFilter })
      console.log("respponse",response);
      const jsonArrayWithId = response?.data?.map((obj, index) => ({ ...obj, id: index + 1 }));
      setData(jsonArrayWithId)
      setRowCount(response.count[0].count)
  ...
  window.NotificationUtils.showError("Error While Recieving Data Please Wait and try again");  
    ...
    // setQueryOptions({ filterModel: { ...filterModel } });
        ...
        let result= [
            ...
            },
            {
              ...
              width: 200,
              filterable: false,
              valueGetter:(params)=>params.row?.farmer_name,
              renderCell: (params) => {
                let val = params.row?.farmer_name||"N/A";
                return <Tooltip title={val}>{val}</Tooltip>;
    ...
    filterable: false, 
            ...
            sortable: false,
            valueGetter: params => {
                const value = params.row?.brand__c||"N/A"
                return value
            },
            renderCell: params => {
              // console.log(params.row.quotation.attributes.shippingOrders.parent.id)
            ...
            {
              field: "iil_product_promoted__c",
              headerName: "IIL Product",
              width: 200,
              sortable: false,
              filterable: false, 
              valueGetter: params =>params.row?.iil_product_promoted__c || 'N/A',
              renderCell: params => {
                const value = params.row?.iil_product_promoted__c || 'N/A'
                return <Tooltip title={value}>{value}</Tooltip>
              },filterOperators: stringOperators 
        
            },
            {
              field: "product_interested__c",
              headerName: "Product Intrested",
              width: 200,
              sortable: false,
              filterable: false, 
              valueGetter: params => params.row?.product_interested__c || 'N/A',
              renderCell: params => {
                const value = params.row?.product_interested__c || 'N/A'
                return <Tooltip title={value}>{value}</Tooltip>
              },
              filterOperators: stringOperators 
            ...
            {
                field: "product_bought__c",
                headerName: "Product Bought",
                width: 200,
                sortable: false,
                filterable: false, 
                valueGetter: params => params.row?.product_bought__c===0?0:params.row?.product_bought__c || 'N/A',
                renderCell: params => {
                  const value = params.row?.product_bought__c===0?0:params.row?.product_bought__c || 'N/A'
                  return <Tooltip title={value}>{value}</Tooltip>
                },
                filterOperators: stringOperators 
              ...
              },
              {
                ...
                renderCell: params => {
                  const value = params.row?.product_quantity__c || 'N/A'
                  return <Tooltip title={value}>{value}</Tooltip>
                },
                filterOperators: stringOperators 
              ...
              {
                field: "on_field__c",
                headerName: "On Field Visit",
                width: 200,
                sortable: false,
                filterable: false, 
                valueGetter: params => params.row?.on_field__c || 'N/A',
                renderCell: params => {
                  const value = params.row?.on_field__c || 'N/A'
                  return <Tooltip title={value}>{value}</Tooltip>
                },
                filterOperators: stringOperators 
...
console.log("Current Date:check1");
  ...
  const response = await window.Platform.database.getFarmerVisitFilter({filterField:searchTerm?.field,filterValue:searchTerm?.value,pageNumber:paginationModel.page,startDate:formattedStartDate,endDate:formattedCurrentDate,territoryName:territoryFilter })
  const jsonArrayWithId = response?.data?.map((obj, index) => ({ ...obj, id: index + 1 }));
  setData(jsonArrayWithId)
  // setData(response.items);
  setRowCount(response.count[0].count)  
...
console.log("Current Date:check2");
  ...
  const response = await window.Platform.database.getFarmerVisitFilter({filterField:"ytd",filterValue:searchTerm,pageNumber:paginationModel.page,startDate:formattedStartDate,endDate:formattedCurrentDate,territoryName:territoryFilter })
  const jsonArrayWithId = response?.data?.map((obj, index) => ({ ...obj, id: index + 1 }));
      setData(jsonArrayWithId)
      // setData(response.items);
      setRowCount(response.count[0].count)
...
setEndDate(formattedCurrentDate)
if(searchTerm){
  const response = await window.Platform.database.getFarmerVisitFilter({filterField:searchTerm.field,filterValue:searchTerm.value,pageNumber:paginationModel.page,startDate:formattedStartOfMonth,endDate:formattedCurrentDate,territoryName:territoryFilter })
  const jsonArrayWithId = response?.data?.map((obj, index) => ({ ...obj, id: index + 1 }));
  setData(jsonArrayWithId)
  // setData(response.items);
  setRowCount(response.count[0].count)  
}else{
  const response = await window.Platform.database.getFarmerVisitFilter({filterField:"mtd",filterValue:searchTerm,pageNumber:paginationModel.page,startDate:formattedStartOfMonth,endDate:formattedCurrentDate,territoryName:territoryFilter })
  const jsonArrayWithId = response?.data?.map((obj, index) => ({ ...obj, id: index + 1 }));
      setData(jsonArrayWithId)
      // setData(response.items);
      setRowCount(response.count[0].count)
...
console.log("Current Date:", formattedCurrentDate);
if(searchTerm){
const response = await window.Platform.database.getFarmerVisitFilter({filterField:searchTerm.field,filterValue:searchTerm.value,pageNumber:paginationModel.page,startDate:formattedCurrentDate,territoryName:territoryFilter })
const jsonArrayWithId = response?.data?.map((obj, index) => ({ ...obj, id: index + 1 }));
      setData(jsonArrayWithId)
      // setData(response.items);
      setRowCount(response.count[0].count)
}else{
const response = await window.Platform.database.getFarmerVisitFilter({filterField:"ftd",filterValue:searchTerm,pageNumber:paginationModel.page,startDate:formattedCurrentDate,territoryName:territoryFilter })
const jsonArrayWithId = response?.data?.map((obj, index) => ({ ...obj, id: index + 1 }));
      setData(jsonArrayWithId)
      // setData(response.items);
      setRowCount(response.count[0].count)
...
try{
  ...
  const response = await window.Platform.database.getFarmerVisitFilter({filterField:searchTerm.field,filterValue:searchTerm?.value,pageNumber:paginationModel.page,startDate:startDate,endDate:endDate,territoryName:data })
  const jsonArrayWithId = response?.data?.map((obj, index) => ({ ...obj, id: index + 1 }));
  setData(jsonArrayWithId)
  // setData(response.items);
  setRowCount(response.count[0].count)  
  ...
  const response = await window.Platform.database.getFarmerVisitFilter({filterField:"territory",filterValue:searchTerm?.value,pageNumber:paginationModel.page,startDate:startDate,endDate:endDate,territoryName:data })
  const jsonArrayWithId = response?.data?.map((obj, index) => ({ ...obj, id: index + 1 }));
  setData(jsonArrayWithId)
  // setData(response.items);
  setRowCount(response.count[0].count)
...
if(searchTerm){
  const response = await window.Platform.database.getFarmerVisitFilter({filterField:searchTerm.field,filterValue:searchTerm?.value,pageNumber:paginationModel.page,startDate:formattedDate,endDate:endDate,territoryName:territoryFilter })
  const jsonArrayWithId = response?.data?.map((obj, index) => ({ ...obj, id: index + 1 }));
  setData(jsonArrayWithId)
  // setData(response.items);
  setRowCount(response.count[0].count)
  ...
  const response = await window.Platform.database.getFarmerVisitFilter({filterField:"territory",filterValue:searchTerm?.value,pageNumber:paginationModel.page,startDate:formattedDate,endDate:endDate,territoryName:territoryFilter })
  const jsonArrayWithId = response?.data?.map((obj, index) => ({ ...obj, id: index + 1 }));
  setData(jsonArrayWithId)
  // setData(response.items);
  setRowCount(response.count[0].count)
...
const finalDateRangeFilter=async(data)=>{
  ...
  setEndDate1(formattedDate)
    ...
    if(searchTerm){
    const response = await window.Platform.database.getFarmerVisitFilter({filterField:searchTerm.field,filterValue:searchTerm?.value,pageNumber:paginationModel.page,startDate:startDate,endDate:data?formattedDate:endDate,territoryName:territoryFilter })
    const jsonArrayWithId = response?.data?.map((obj, index) => ({ ...obj, id: index + 1 }));
    setData(jsonArrayWithId)
    // setData(response.items);
    setRowCount(response.count[0].count)  
    ...
    const response = await window.Platform.database.getFarmerVisitFilter({filterField:"territory",filterValue:searchTerm?.value,pageNumber:paginationModel.page,startDate:startDate,endDate:data?formattedDate:endDate,territoryName:territoryFilter })
    const jsonArrayWithId = response?.data?.map((obj, index) => ({ ...obj, id: index + 1 }));
    setData(jsonArrayWithId)
    // setData(response.items);
    setRowCount(response.count[0].count)
...
const clearDateFilter=async()=>{
  ...
  if(searchTerm||territoryFilter){
    try{
    setLoading(true)
    const response = await window.Platform.database.getFarmerVisitFilter({filterField:searchTerm?searchTerm.field:"territory",filterValue:searchTerm?.value,pageNumber:paginationModel.page,startDate:'',endDate:'',territoryName:territoryFilter })
    const jsonArrayWithId = response?.data?.map((obj, index) => ({ ...obj, id: index + 1 }));
    setData(jsonArrayWithId)
    // setData(response.items);
    setRowCount(response.count[0].count)
...
const clearTerritoryFIlter=async()=>{
  ...
  if(searchTerm||startDate||endDate){
    try{
    setLoading(true)
    const response = await window.Platform.database.getFarmerVisitFilter({filterField:searchTerm?searchTerm.field:"territory",filterValue:searchTerm?.value,pageNumber:paginationModel.page,startDate:startDate,endDate:endDate,territoryName:'' })
    const jsonArrayWithId = response?.data?.map((obj, index) => ({ ...obj, id: index + 1 }));
    setData(jsonArrayWithId)
    // setData(response.items);
    setRowCount(response.count[0].count)
  ...
  return (
    <>
      ...
      <StaffEditorPageContainer>
        <HeaderContainer>
          <Typography variant="h5">Farmer Visits</Typography>
